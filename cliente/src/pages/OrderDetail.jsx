...
const OrderDetail = () => {
  ...
  const fetchPedido = async () => {
    try {
      ...
      // Only try to fetch history if order is shipped or delivered
      if (res.data.data.estadoPedido === 'enviado' || res.data.data.estadoPedido === 'entregado') {
        try {
          const historialRes = await axios.get(`/api/pedidos/${id}/historial`, config);
          setSeguimiento({
            numeroSeguimiento: res.data.data.numeroSeguimiento,
            empresa: res.data.data.empresaEnvio,
            fechaEnvio: res.data.data.fechaEnvio,
            urlSeguimiento: res.data.data.urlSeguimiento,
            estimatedDelivery: res.data.data.fechaEstimadaEntrega,
            historia: historialRes.data.data || []
          });
        } catch (historyErr) {
          console.warn('No se pudo cargar el historial:', historyErr);
          // Still set tracking info without history
          setSeguimiento({
            numeroSeguimiento: res.data.data.numeroSeguimiento,
            empresa: res.data.data.empresaEnvio,
            fechaEnvio: res.data.data.fechaEnvio,
  ...
  const handleSubirComprobante = async () => {
    ...
    try {
      ...
      setPedido(prev => ({
        ...
        estadoPago: 'pendiente' // Or whatever status is appropriate after submission
      }));
  ...
  const generatePDF = async () => {
    try {
      ...
      const estadoPedidoInfoPdf = getEstadoPedidoInfo(pedido.estadoPedido);
      const estadoPagoInfoPdf = getEstadoPagoInfo(pedido.estadoPago);
      doc.text(`Estado del pedido: ${estadoPedidoInfoPdf.text}`, margin, yPos);
      yPos += 6;
      doc.text(`Estado del pago: ${estadoPagoInfoPdf.text}`, margin, yPos);
      yPos += 15;
      ...
      generateScreenshotPDF(); // Fallback
    }
  };

  const generateScreenshotPDF = async () => {
    try {
      const element = document.getElementById('order-detail-content-for-pdf'); // Use a specific wrapper for PDF content
      if (!element) {
      ...
      console.error('Error al generar PDF de captura de pantalla:', err);
    }
  };
  ...
  return (
    <div className="bg-gray-50 min-h-screen py-10">
      <div className="max-w-6xl mx-auto px-4">
        ...
        {/* This div wraps the content that html2canvas will capture */}
        <div id="order-detail-content-for-pdf">
            {!isCancelled && (
            ...
            <div className="lg:col-span-1 space-y-8">
                ...
                </div> {/* Closes Resumen del pedido card */}
                ...
                </div> {/* Closes Dirección de Envío card */}
                ...
                </div> {/* Closes Método de Pago card */}
                ...
                </div> {/* Closes ¿Necesitas ayuda? card */}
            </div> {/* Closes lg:col-span-1 */}
            </div> {/* Closes id="order-detail" grid */}
        </div> {/* Closes id="order-detail-content-for-pdf" */}
      </div> {/* Closes max-w-6xl */}
    </div> /* Closes bg-gray-50 */
  );
};
