...
// @desc    Exportar reportes a Excel
...
export const exportarReporte = async (req, res) => {
  try {
    ...
    workbook.creator = 'Sistema de Tienda';
    workbook.lastModifiedBy = 'Sistema de Tienda';
    ...
    switch (tipo) {
      case 'mensual':
        await generarReporteMensual(mes, anio, workbook);
        break;
      case 'ventas':
        await generarReporteVentas(mes, anio, workbook);
        break;
      case 'productos':
        await generarReporteProductos(mes, anio, workbook);
        break;
      case 'clientes':
        await generarReporteClientes(mes, anio, workbook);
        break;
      default:
        return res.status(400).json({
          success: false,
...
async function generarReporteMensual(mes, anio, workbook) {
  ...
  const ventasMesAnterior = pedidosMesAnterior.reduce((sum, p) => sum + (p.total || 0), 0);
...
async function generarReporteProductos(mes, anio, workbook) {
  ...
  const pedidos = await Pedido.find({
    createdAt: { $gte: fechaInicio, $lte: fechaFin }
  }).populate('productos.producto', 'nombre precio categoria stock');
